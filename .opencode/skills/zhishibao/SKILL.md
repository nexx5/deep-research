...
- **嵌入 API**：需要一个 OpenAI 兼容的 embeddings 端点（用于向量检索）。在**运行包根目录的 `.env`** 中配置（复制 `.env.example` 为 `.env` 后填写）：`EMBED_BASE_URL` / `EMBED_API_KEY` / `EMBED_MODEL`。兼容旧方式：项目 `knowledge-pack/config.json` 的 `embed` 段或环境变量（EMBED_*/QMD_OPENAI_*）仍可兜底。配置优先级：`.env` > 环境变量 > config.json > 默认值
...
自动创建：SQLite 数据库 + 空 claims.jsonl + 空 relations.jsonl + config.json 模板（从 skill 目录复制，embed 段为空占位）。嵌入 API 凭据统一放运行包根目录 `.env`。

## 配置（嵌入 API 凭据）

**推荐方式：运行包根目录 `.env`**（复制 `.env.example` 为 `.env` 后填写）：

```dotenv
EMBED_BASE_URL=http://your-embed-host:port/v1
EMBED_API_KEY=sk-your-key-here
EMBED_MODEL=qwen3-embed
```

兼容兜底（`.env` 未配置时依次尝试）：环境变量（EMBED_*/QMD_OPENAI_*）> 项目 `<项目根>/knowledge-pack/config.json` 的 `embed` 段 > 脚本默认值（占位，不含真实凭据）。
...
配置优先级：`.env` > 环境变量 > config.json > 默认值。
