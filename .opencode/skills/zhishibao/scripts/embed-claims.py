...
配置优先级：.env(运行包根) > 环境变量(EMBED_*/QMD_OPENAI_*) > config.json(兼容) > 默认值
密钥等隐私信息放在运行包根目录 .env，禁止硬编码
"""
...
# ============================================================
# 配置读取：.env > 环境变量 > config.json > 默认值
# 密钥等隐私信息统一放在运行包根目录的 .env，禁止硬编码
# ============================================================

def load_env_file():
    """从运行包根目录加载 .env（不覆盖已存在的环境变量）。"""
...
def load_embed_config(project_path):
    ...
    # 环境变量（含 .env 注入）优先
    base_url = os.environ.get('EMBED_BASE_URL') or os.environ.get('QMD_OPENAI_BASE_URL') or ''
    api_key = os.environ.get('EMBED_API_KEY') or os.environ.get('QMD_OPENAI_API_KEY') or ''
    model = os.environ.get('EMBED_MODEL') or os.environ.get('QMD_EMBED_MODEL') or ''

    # config.json 仅作兼容兜底（.env 未配置时）
    if os.path.exists(config_path):
        with open(config_path, 'r', encoding='utf-8') as f:
            cfg = json.load(f)
        embed = cfg.get('embed', {})
        if not base_url and embed.get('base_url'):
            base_url = embed['base_url']
        if not api_key and embed.get('api_key'):
            api_key = embed['api_key']
        if not model and embed.get('model'):
            model = embed['model']
