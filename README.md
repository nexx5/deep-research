...
>
> 版本记录见 [CHANGELOG.md](CHANGELOG.md)，升级指引见 [升级说明.md](升级说明.md)。
...
| 调研员 | 方案批准后作为**调度器**运行：强制 MapReduce 派发采录员 subagent（采集+采录+单源分析），禁止自己直采；负责跨源对比与质量闸门，并挖掘新线索。 |
...
调研员按 **MapReduce 模式**执行：采集+采录+单源分析强制派发采录员 subagent（并发 4~12，默认 6；禁止直采），跨源对比与质量闸门由调研员批次内直接执行；知识体系维护由知识管理员负责。
...
- **请斟酌使用**：本项目调研时长较长，知识处理过程消耗偏大。这是知识积累的代价——为换取可追溯、可复用的知识资产，需要投入持续的采集、分析、校验与维护成本；对只需一次性快速结论的场景，建议优先使用「快速调研」模式。
...
此外，本项目的调研方法论、工作流设计与技术思路广泛借鉴了storm-research、local-deep-research 等众多开源项目与社区实践，受益良多，在此一并致谢，恕不一一列举。
